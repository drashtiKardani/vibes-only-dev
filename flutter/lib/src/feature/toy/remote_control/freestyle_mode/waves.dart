...
class _WaveWidgetState extends State<WaveWidget> with TickerProviderStateMixin {
  ...
  void _initAnimations() {
    if (widget.config.colorMode == ColorMode.custom) {
      _waveControllers = (widget.config as CustomConfig).durations!.map((
        duration,
      ) {
        // Calculate amplitude based on intensity
        double amplitude = (pow(widget.intensity, 2) / 100 + 5);

        _waveAmplitudes.add(amplitude);
  ...
  void didUpdateWidget(WaveWidget oldWidget) {
    ...
    if (widget.intensity != oldWidget.intensity) {
      for (var i = 0; i < _waveAmplitudes.length; i++) {
        _waveAmplitudes[i] = (pow(widget.intensity, 2) / 100 + 5);
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(children: _buildPaints()),
    );
  }
}
...
class _CustomWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    double viewCenterY = size.height / 2 * (heightPercentage! + 0.1);
