...
class UpdateStaffPage extends StatefulWidget {
  const UpdateStaffPage({super.key, @PathParam('id') required this.id});
...
class _UpdateStaffPageState extends State<UpdateStaffPage> {
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<CrudCubit, CrudState>(
        ...
        listener: (blocContext, state) {
          state.maybeWhen(
            getStaff: (staff) {
              _firstNameTextEditingController.text = staff.firstName ?? '';
              _lastNameTextEditingController.text = staff.lastName ?? '';
              _emailTextEditingController.text = staff.email ?? '';
              _phoneNumberTextEditingController.text = staff.phoneNumber ?? '';
            },
            successfulCreate: () {
        ...
        builder: (context, state) => CrudScaffold(
              ...
              title: S.of(context).updateStaff,
  ...
  void _submitForm() async {
    if (_validateFields()) {
      cubit.updateStaff(widget.id, _firstNameTextEditingController.text, _lastNameTextEditingController.text,
          _passwordTextEditingController.text, _phoneNumberTextEditingController.text);
    }
  }
